...
export const createEventSchema = z.object({
  body: z.object({
    ...
    start_time:     z.string().regex(/^([0-1][0-9]|2[0-3]):[0-5][0-9](:[0-5][0-9])?$/, 'Invalid time format (HH:MM)').optional(),
    end_time:       z.string().regex(/^([0-1][0-9]|2[0-3]):[0-5][0-9](:[0-5][0-9])?$/, 'Invalid time format (HH:MM)').optional(),
...
export type CalendarFiltersInput = z.infer<typeof calendarFiltersSchema>['query'];